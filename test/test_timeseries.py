...
def test_mean_by_window():
    ...
    test1 = data.meanByWindow(indices=[3, 5], window=2).toarray()
    assert allclose(test1, [3, 4])
    test2 = data.meanByWindow(indices=[3, 5], window=3).toarray()
    assert allclose(test2, [3, 4, 5])
    test3 = data.meanByWindow(indices=[3, 5], window=4).toarray()
    assert allclose(test3, [2, 3, 4, 5])
    test4 = data.meanByWindow(indices=[3], window=4).toarray()
    assert allclose(test4, [1, 2, 3, 4])


def test_group_by_window():
    data = fromList([array([0, 1, 2, 3, 4, 5, 6])]).toTimeSeries()
    test1 = data.groupByWindow(indices=[3, 5], window=2).toarray()
    assert allclose(test1, [[2, 3], [4, 5]])
    test2 = data.groupByWindow(indices=[3, 5], window=3).toarray()
    assert allclose(test2, [[2, 3, 4], [4, 5, 6]])
    test3 = data.groupByWindow(indices=[3, 5], window=4).toarray()
    assert allclose(test3, [[1, 2, 3, 4], [3, 4, 5, 6]])
    test4 = data.groupByWindow(indices=[3, 4, 5], window=2).toarray()
    assert allclose(test4, [[2, 3], [3, 4], [4, 5]])
    test5 = data.groupByWindow(indices=[3], window=2).toarray()
