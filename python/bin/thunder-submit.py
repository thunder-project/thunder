...
def main():
    ...
    from optparse import OptionParser, OptionGroup  # use optparse instead of argparse for python2.6 compat
    ...
    parser.add_option("--master", default=getMasterURI(),
                      help="spark://host:port, mesos://host:port, yarn, or local (default: '%default').")
    parser.add_option("--deploy-mode", default="client", type="choice", choices=["client", "cluster"],
    ...
    parser.add_option("--py-files", default="",
    ...
    parser.add_option("--jars", default="",
    ...
    uncommonGroup = OptionGroup(parser, "Less Common Options")
    ...
    uncommonGroup.add_option(
        "--properties-file", default=os.path.join(SPARK_HOME, "conf", "spark-defaults.conf"),
    ...
    parser.add_option_group(uncommonGroup)
    standaloneClusterGroup = OptionGroup(parser, "Spark standalone with cluster deploy mode only")
    ...
    parser.add_option_group(standaloneClusterGroup)
    standaloneMesosGroup = OptionGroup(parser, "Spark standalone and Mesos only")
    standaloneMesosGroup.add_option("--total-executor-cores", default=-1, type="int",
                                    help="Total cores for all executors.")
    parser.add_option_group(standaloneMesosGroup)
    yarnGroup = OptionGroup(parser, "YARN-only")
    ...
    yarnGroup.add_option("--archives", default="",
                         help="Comma separated list of archives to be extracted into the working directory " +
                              "of each executor.")
    parser.add_option_group(yarnGroup)
