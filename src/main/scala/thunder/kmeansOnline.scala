...
object kmeansOnline {
  ...
  def printToImage2(rdd: RDD[Double], width: Int, height: Int, fileName: String): Unit = {
    ...
    val nums = rdd.map(x => clip(((x/3000-1)*255).toInt)).collect()
  ...
  def main(args: Array[String]) {
    ...
    var centersForPrinting = List(List(makeMap(centers(0).elements)))
    for (ik <- 1 until k) {
      centersForPrinting = centersForPrinting ++ List(List(makeMap(centers(ik).elements)))
    }
